...
def multiple_choice_flow():
    ...
    while True:
        ...
        continue_input = input("Do you want to continue choice flow? (yes/no): ")
...
def question_ans_flow():
    ...
    while True:
        ...
        print(f"Debug: Model result: {result}")
        add_to_context(result)
        continue_input = input("Do you want to continue question flow? (yes/no): ")
        if continue_input.lower() == "yes":
            continue
        else:
            break;
...
def handle_conversation():
    read_context_from_file('context.txt')
    print("Welcome to Mobot! Type 'exit' to quit")
    while True:
        opt = input("Do you want to QnA or Multiple choice QnA (choice/quest/refres-ctx) or exit to quit: ")
        if opt.lower() == "choice":
            multiple_choice_flow()
        elif opt.lower() == "quest":
            question_ans_flow()
        elif opt.lower() == "refresh-ctx":
